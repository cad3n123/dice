...
export function farkleScoring(dice: Ref<DiceItem[]>, scoring: any) {
  const unplayedHands = computed((): FoundHand[] => {
    // 1. Filter and group the "selected" dice by their value.
    const selectedDice = dice.value.filter((d) => d.list !== 'played');
    ...
    let diceRemaining = selectedDice.length;
    let ruleIndex = 0;
  ...
  const selectedHands = computed((): FoundHand[] => {
    // 1. Filter and group the "selected" dice by their value.
